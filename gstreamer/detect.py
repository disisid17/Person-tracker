    ...
    if trackerFlag and (np.array(trdata)).size:
        for td in trdata:
            ...
            if labels.get(obj.id, obj.id) == 'person':
                ...
                x, y, w, h = x * scale_x + w*scale_x/2, y * scale_y+ h*scale_y/2, w * scale_x/10, h * scale_y/10
                percent = int(100 * obj.score)
                label = '{}% {} ID:{}'.format(
                    percent, labels.get(obj.id, obj.id), int(trackID))
                shadow_text(dwg, x, y - 5, label)
                dwg.add(dwg.rect(insert=(x, y), size=(10, 10),
        ...
        for obj in objs:
            ...
            if labels.get(obj.id, obj.id) == 'person':
                ...
                shadow_text(dwg, x, y - 5, label)
                dwg.add(dwg.rect(insert=(x, y), size=(10, 10), fill='none', stroke='red', stroke_width='5'))
    text_lines[2] = '# of people: {}'.format(count)
    for y, line in enumerate(text_lines, start=1):
        shadow_text(dwg, 10, y*20, line)
    #print(dwg.tostring())
    #print("\n")
    return dwg.tostring(),count
...
def main():
    ...
    def user_callback(input_tensor,src_size, inference_box, mot_tracker):
        ...
        if len(objs) != 0:
            imga,count = generate_svg(src_size, inference_size, inference_box, objs, labels, text_lines, trdata, trackerFlag)
            #print(count)
            return imga,count
        return "",0
